...
export default()=>{
    ...
    useEffect(() => {
        const fetchData= async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/foodlink/foodbank/volunteers/67acd88a4540909a08d6cb73?status=approved`, {
                    method: 'GET',
                    headers: {
              ...
              {Array.isArray(data) && data.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="border p-2 flex items-center justify-center">
                    ...
                    <button className="ml-2 bg-blue-500 text-white px-2 py-1 rounded text-sm">View Detail</button>
                  </td>
