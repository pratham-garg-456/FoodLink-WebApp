...
//     const directionsUrl = `https://api.mapbox.com/directions/v5/mapbox/driving/${userCoords[0]},${userCoords[1]};${foodBankCoords[0]},${foodBankCoords[1]}?geometries=geojson&steps=true&access_token=${mapboxgl.accessToken}`;
...
//         // Extract the route's geometry coordinates and steps
//         const routeCoordinates = data.routes[0].geometry.coordinates;
//         const routeSteps = data.routes[0].legs[0].steps.map(step => step.maneuver.instruction);
//         setDirections({ routeCoordinates, routeSteps });
//       } else {
  ...
  const getDirections = async (foodBank) => {
    ...
    try {
      ...
      if (data.routes && data.routes[0]) {
        ...
        setDirections({ steps, route }); // Pass both route and steps
      } else {
        alert('No directions found.');
      }
    } catch (error) {
...
export async function getServerSideProps() {
  // Fetch food banks data from an API or database
  const foodBanks = [
