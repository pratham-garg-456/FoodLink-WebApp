...
async def get_appointments_by_foodbank(foodbank_id: str):
    ...
    """
    try:
        appointments = await Appointment.find(Appointment.foodbank_id == foodbank_id).to_list()
        ...
        raise HTTPException(
            status_code=500, detail=f"An error occurred while fetching appointments: {str(e)}"
        )
...
async def update_appointment_status_in_db(appointment_id: str, updated_status: str):
    ...
    if updated_status == "cancelled":
        existing_inventory = await MainInventory.find_one(MainInventory.foodbank_id == appointment.foodbank_id)

        if not existing_inventory:
            raise HTTPException(status_code=404, detail="Inventory not found for this food bank.")
        ...
        for item in appointment.product:
            ...
            if not food_found:
                existing_inventory.stock.append(AppointmentFoodItem(food_name=food_name, quantity=quantity))
...
async def check_time_slot_availability(foodbank_id: str, start_time: datetime, end_time: datetime) -> bool:
    """
    ...
    overlapping_appointments = await Appointment.find(
        {
            ...
            "start_time": {"$lt": end_time},  # Appointments that start before the new end time
            "end_time": {"$gt": start_time}  # Appointments that end after the new start time
        }
    ).to_list()

    return len(overlapping_appointments) == 0  # If no overlapping appointments, the slot is available
...
async def create_an_event_in_db(foodbank_id: str, event_data: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"An error occurred while creating an event in db: {str(e)}")
...
async def get_event_inventory_from_db(event_id: str):
    ...
    try:
        ...
        event_inventory = await EventInventory.find_one(EventInventory.event_id == event_id)
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving event inventory: {str(e)}")
...
async def add_event_inventory_to_db(event_id: str, foodbank_id: str, stock_data: List[dict]):
    """
    ...
    try:
        ...
        main_inventory = await MainInventory.find_one(MainInventory.foodbank_id == foodbank_id)
        ...
        if not event:
            raise HTTPException(
                status_code=404,
                detail=f"Event not found for ID: {event_id}."
            )
        ...
        for item in stock_data:
            ...
            if not event_food_found:
                ...
                event_inventory.stock.append(EventInventoryFoodItem(food_name=food_name, quantity=quantity))
        ...
        raise HTTPException(status_code=500, detail=f"An error occurred while adding inventory to event: {str(e)}")
...
async def update_event_inventory_in_db(event_id: str, used_items: List[dict]):
    ...
    try:
        ...
        for used_item in used_items:
            event_item = next((ei for ei in event_inventory.stock if ei.food_name == used_item["food_name"]), None)
        ...
        raise HTTPException(status_code=500, detail=f"Error updating event inventory in db: {str(e)}")


async def transfer_event_inventory_to_main_inventory_in_db( foodbank_id: str, event_id: str):
    """
    ...
    try:
        ...
        if not event_inventory or not event_inventory.stock:
            raise HTTPException(status_code=404, detail="No inventory to transfer back.")
        ...
        for item in event_inventory.stock:
            main_item = next((mi for mi in main_inventory.stock if mi.food_name == item.food_name), None)
            if main_item:
        ...
        raise HTTPException(status_code=500, detail=f"Error transferring inventory back: {str(e)}")
